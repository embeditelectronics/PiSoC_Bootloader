...
#if((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u))
    static uint8 clearInDataRdyStatus = USB_Bootloader_ARB_EPX_CFG_DEFAULT;
    uint8 USB_Bootloader_DmaNextTd[USB_Bootloader_MAX_EP];
    const uint8 USB_Bootloader_epX_TD_TERMOUT_EN[USB_Bootloader_MAX_EP] =
    {   0u,
        USB_Bootloader_ep1_TD_TERMOUT_EN,
        USB_Bootloader_ep2_TD_TERMOUT_EN,
        USB_Bootloader_ep3_TD_TERMOUT_EN,
        USB_Bootloader_ep4_TD_TERMOUT_EN,
        USB_Bootloader_ep5_TD_TERMOUT_EN,
        USB_Bootloader_ep6_TD_TERMOUT_EN,
        USB_Bootloader_ep7_TD_TERMOUT_EN,
        USB_Bootloader_ep8_TD_TERMOUT_EN
    };
    volatile uint16 USB_Bootloader_inLength[USB_Bootloader_MAX_EP];
    const uint8 *USB_Bootloader_inDataPointer[USB_Bootloader_MAX_EP];
    volatile uint8 USB_Bootloader_inBufFull[USB_Bootloader_MAX_EP];
#endif /* ((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u)) */
...
void USB_Bootloader_Init(void) 
{
    ...
    */
    USB_Bootloader_PM_USB_CR0_REG &= ((uint8)(~(USB_Bootloader_PM_USB_CR0_PD_N |
                                                  USB_Bootloader_PM_USB_CR0_PD_PULLUP_N)));

    /* Select iomode to USB mode*/
    USB_Bootloader_USBIO_CR1_REG &= ((uint8)(~USB_Bootloader_USBIO_CR1_IOMODE));
    ...
    USB_Bootloader_DM_INP_DIS_REG &= ((uint8)(~USB_Bootloader_DM_MASK));
    USB_Bootloader_DP_INP_DIS_REG &= ((uint8)(~USB_Bootloader_DP_MASK));
    ...
    /* Write WAx */
    CY_SET_REG8(USB_Bootloader_ARB_RW1_WA_PTR,     0u);
    CY_SET_REG8(USB_Bootloader_ARB_RW1_WA_MSB_PTR, 0u);
        ...
        for (i = 0u; i < USB_Bootloader_MAX_EP; i++)
        {
            ...
            #if ((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u))
                USB_Bootloader_DmaNextTd[i] = DMA_INVALID_TD;
            #endif /* ((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u)) */
        }
    #endif   /*  USB_Bootloader_EP_MM != USB_Bootloader__EP_MANUAL */
    ...
    (void) CyIntSetVector(USB_Bootloader_BUS_RESET_VECT_NUM,   &USB_Bootloader_BUS_RESET_ISR);
    ...
    /* Set the SOF Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_SOF_VECT_NUM,   &USB_Bootloader_SOF_ISR);
    ...
    (void) CyIntSetVector(USB_Bootloader_EP_0_VECT_NUM,   &USB_Bootloader_EP_0_ISR);
    ...
    /* Set the Data Endpoint 1 Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_EP_1_VECT_NUM,   &USB_Bootloader_EP_1_ISR);
    ...
    /* Set the Data Endpoint 2 Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_EP_2_VECT_NUM,   &USB_Bootloader_EP_2_ISR);
    ...
    /* Set the Data Endpoint 3 Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_EP_3_VECT_NUM,   &USB_Bootloader_EP_3_ISR);
    ...
    /* Set the Data Endpoint 4 Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_EP_4_VECT_NUM,   &USB_Bootloader_EP_4_ISR);
    ...
    /* Set the Data Endpoint 5 Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_EP_5_VECT_NUM,   &USB_Bootloader_EP_5_ISR);
    ...
    /* Set the Data Endpoint 6 Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_EP_6_VECT_NUM,   &USB_Bootloader_EP_6_ISR);
     ...
     /* Set the Data Endpoint 7 Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_EP_7_VECT_NUM,   &USB_Bootloader_EP_7_ISR);
    ...
    /* Set the Data Endpoint 8 Interrupt. */
        ...
        (void) CyIntSetVector(USB_Bootloader_EP_8_VECT_NUM,   &USB_Bootloader_EP_8_ISR);
        ...
        (void) CyIntSetVector(USB_Bootloader_ARB_VECT_NUM,   &USB_Bootloader_ARB_ISR);
...
*
* Global variables:
*   USB_Bootloader_device: Contains the device number of the desired device
...
void USB_Bootloader_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
        ...
        {
            USB_Bootloader_hidProtocol[i] = USB_Bootloader_PROTOCOL_REPORT;
        }
    ...
    /* Arbiter configuration for DMA transfers */
            ...
            #if(USB_Bootloader_EP_DMA_AUTO_OPT == 0u)
                /* Init interrupt which handles verification of the successful DMA transaction */
                USB_Bootloader_EP_DMA_Done_isr_StartEx(&USB_Bootloader_EP_DMA_DONE_ISR);
                USB_Bootloader_EP17_DMA_Done_SR_InterruptEnable();
                USB_Bootloader_EP8_DMA_Done_SR_InterruptEnable();
            #endif /* USB_Bootloader_EP_DMA_AUTO_OPT == 0u */
    ...
    switch(mode)
    {
        ...
        default:   /*USB_Bootloader_DWR_VDDD_OPERATION */
            ...
            #else
                USB_Bootloader_CR1_REG = USB_Bootloader_CR1_ENABLE_LOCK | USB_Bootloader_CR1_REG_ENABLE;
            #endif /*  USB_Bootloader_VDDD_MV < USB_Bootloader_3500MV */
            break;
    }
    ...
    USB_Bootloader_interfaceNumber = 0u;
...
void USB_Bootloader_Stop(void) 
{
    ...
    USB_Bootloader_USBIO_CR1_REG &= (uint8)(~USB_Bootloader_USBIO_CR1_USBPUEN);
...
uint8 USB_Bootloader_CheckActivity(void) 
{
    ...
    CY_SET_REG8(USB_Bootloader_CR1_PTR, (r & ((uint8)(~USB_Bootloader_CR1_BUS_ACTIVITY))));
...
uint16 USB_Bootloader_GetEPCount(uint8 epNumber) 
{
    ...
    if((epNumber > USB_Bootloader_EP0) && (epNumber < USB_Bootloader_MAX_EP))
    {
        ...
        result = (uint8)(CY_GET_REG8((reg8 *)(USB_Bootloader_SIE_EP1_CNT0_IND + ri)) &
                          USB_Bootloader_EPX_CNT0_MASK);
        result = (result << 8u) | CY_GET_REG8((reg8 *)(USB_Bootloader_SIE_EP1_CNT1_IND + ri));
        result -= USB_Bootloader_EPX_CNTX_CRC_COUNT;
    }
    return(result);
}
    ...
    *  USB_Bootloader_LoadInEP() or USB_Bootloader_ReadOutEP() APIs for data
    *  transfer.
    *
    * Parameters:
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        if((epNumber > USB_Bootloader_EP0) && (epNumber < USB_Bootloader_MAX_EP))
        {
            ...
            #if ((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u))
                USB_Bootloader_DmaNextTd[epNumber] = CyDmaTdAllocate();
            #endif /*  ((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u)) */
    ...
    *******************************************************************************/
    void USB_Bootloader_Stop_DMA(uint8 epNumber) 
    {
        ...
        do
        {
            ...
            {
                (void) CyDmaChDisable(USB_Bootloader_DmaChan[i]);
                CyDmaTdFree(USB_Bootloader_DmaTd[i]);
                USB_Bootloader_DmaTd[i] = DMA_INVALID_TD;
            }
            #if ((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u))
                if(USB_Bootloader_DmaNextTd[i] != DMA_INVALID_TD)
                {
                    CyDmaTdFree(USB_Bootloader_DmaNextTd[i]);
                    USB_Bootloader_DmaNextTd[i] = DMA_INVALID_TD;
                }
            #endif /* ((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u)) */
            i++;
    ...
    /*******************************************************************************
    * Function Name: USB_Bootloader_LoadNextInEP
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void USB_Bootloader_LoadNextInEP(uint8 epNumber, uint8 mode) 
    {
        ...
        if(mode == 0u)
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USB_Bootloader_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length */
            CY_SET_REG16(convert, USB_Bootloader_inLength[epNumber] - USB_Bootloader_DMA_BYTES_PER_BURST);
            /* CyDmaTdSetAddress API is optimized to change only source address */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USB_Bootloader_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert, LO16((uint32)USB_Bootloader_inDataPointer[epNumber] +
                                            USB_Bootloader_DMA_BYTES_PER_BURST));
            USB_Bootloader_inBufFull[epNumber] = 1u;
        }
        else
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USB_Bootloader_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length */
            CY_SET_REG16(convert, USB_Bootloader_DMA_BYTES_REPEAT);
            /* CyDmaTdSetAddress API is optimized to change only source address */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USB_Bootloader_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert,  LO16((uint32)USB_Bootloader_inDataPointer[epNumber] +
                                   USB_Bootloader_DMA_BYTES_PER_BURST - USB_Bootloader_DMA_BYTES_REPEAT));
        }

        /* CyDmaChSetInitialTd API is optimised to init TD */
        CY_DMA_CH_STRUCT_PTR[USB_Bootloader_DmaChan[epNumber]].basic_status[1u] = USB_Bootloader_DmaTd[epNumber];
    }
#endif /* ((USB_Bootloader_EP_MM == USB_Bootloader__EP_DMAAUTO) && (USB_Bootloader_EP_DMA_AUTO_OPT == 0u)) */
                                                                        ...
                                                                        
{
    ...
    {
        ri = ((epNumber - USB_Bootloader_EP1) << USB_Bootloader_EPX_CNTX_ADDR_SHIFT);
        p = (reg8 *)(USB_Bootloader_ARB_RW1_DR_IND + ri);
        ...
        CY_SET_REG8((reg8 *)(USB_Bootloader_SIE_EP1_CNT0_IND + ri),
                            (length >> 8u) | (USB_Bootloader_EP[epNumber].epToggle));
        CY_SET_REG8((reg8 *)(USB_Bootloader_SIE_EP1_CNT1_IND + ri),  length & 0xFFu);
            ...
            CY_SET_REG8((reg8 *)(USB_Bootloader_SIE_EP1_CR0_IND + ri), USB_Bootloader_EP[epNumber].epMode);
        #else
            ...
            if ((pData != NULL) && (length > 0u))
            {
                ...
                (void) CyDmaChDisable(USB_Bootloader_DmaChan[epNumber]);
                (void) CyDmaTdSetConfiguration(USB_Bootloader_DmaTd[epNumber], length, CY_DMA_DISABLE_TD,
                                                                                 TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USB_Bootloader_DmaTd[epNumber],  LO16((uint32)pData), LO16((uint32)p));
                /* Enable the DMA */
                (void) CyDmaChSetInitialTd(USB_Bootloader_DmaChan[epNumber], USB_Bootloader_DmaTd[epNumber]);
                (void) CyDmaChEnable(USB_Bootloader_DmaChan[epNumber], 1u);
                /* Generate DMA request */
                * (reg8 *)(USB_Bootloader_ARB_EP1_CFG_IND + ri) |= USB_Bootloader_ARB_EPX_CFG_DMA_REQ;
                * (reg8 *)(USB_Bootloader_ARB_EP1_CFG_IND + ri) &= ((uint8)(~USB_Bootloader_ARB_EPX_CFG_DMA_REQ));
            ...
            if (pData != NULL)
            {
                ...
                (void) CyDmaChDisable(USB_Bootloader_DmaChan[epNumber]);
            #if (USB_Bootloader_EP_DMA_AUTO_OPT == 0u)
                USB_Bootloader_inLength[epNumber] = length;
                USB_Bootloader_inDataPointer[epNumber] = pData;
                /* Configure DMA to send the data only for the first burst */
                (void) CyDmaTdSetConfiguration(USB_Bootloader_DmaTd[epNumber],
                    (length > USB_Bootloader_DMA_BYTES_PER_BURST) ? USB_Bootloader_DMA_BYTES_PER_BURST : length,
                    USB_Bootloader_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USB_Bootloader_DmaTd[epNumber],  LO16((uint32)pData), LO16((uint32)p));
                ...
                */
                (void) CyDmaTdSetConfiguration(USB_Bootloader_DmaNextTd[epNumber], 1u,
                                               USB_Bootloader_DmaNextTd[epNumber],
                                               USB_Bootloader_epX_TD_TERMOUT_EN[epNumber]);
                /* Configure DmaNextTd to clear Data ready status */
                (void) CyDmaTdSetAddress(USB_Bootloader_DmaNextTd[epNumber],  LO16((uint32)&clearInDataRdyStatus),
                                                                LO16((uint32)(USB_Bootloader_ARB_EP1_CFG_IND + ri)));
            #else /* Configure DMA to send all data*/
                (void) CyDmaTdSetConfiguration(USB_Bootloader_DmaTd[epNumber], length,
                                               USB_Bootloader_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USB_Bootloader_DmaTd[epNumber],  LO16((uint32)pData), LO16((uint32)p));
            #endif /* USB_Bootloader_EP_DMA_AUTO_OPT == 0u */

                /* Clear Any potential pending DMA requests before starting the DMA channel to transfer data */
                (void) CyDmaClearPendingDrq(USB_Bootloader_DmaChan[epNumber]);
                /* Enable the DMA */
                (void) CyDmaChSetInitialTd(USB_Bootloader_DmaChan[epNumber], USB_Bootloader_DmaTd[epNumber]);
                (void) CyDmaChEnable(USB_Bootloader_DmaChan[epNumber], 1u);
            }
            else
            {
                ...
                if(length > 0u)
                {
                #if (USB_Bootloader_EP_DMA_AUTO_OPT == 0u)
                    USB_Bootloader_inLength[epNumber] = length;
                    USB_Bootloader_inBufFull[epNumber] = 0u;
                    (void) CyDmaChDisable(USB_Bootloader_DmaChan[epNumber]);
                    /* Configure DMA to send the data only for the first burst */
                    (void) CyDmaTdSetConfiguration(
                        USB_Bootloader_DmaTd[epNumber], (length > USB_Bootloader_DMA_BYTES_PER_BURST) ?
                        USB_Bootloader_DMA_BYTES_PER_BURST : length,
                        USB_Bootloader_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR );
                    (void) CyDmaTdSetAddress(USB_Bootloader_DmaTd[epNumber],
                                             LO16((uint32)USB_Bootloader_inDataPointer[epNumber]), LO16((uint32)p));
                    /* Clear Any potential pending DMA requests before starting the DMA channel to transfer data */
                    (void) CyDmaClearPendingDrq(USB_Bootloader_DmaChan[epNumber]);
                    /* Enable the DMA */
                    (void) CyDmaChSetInitialTd(USB_Bootloader_DmaChan[epNumber], USB_Bootloader_DmaTd[epNumber]);
                    ...
                    #ifndef USB_Bootloader_MANUAL_IN_EP_ARM
                        * (reg8 *)(USB_Bootloader_ARB_EP1_CFG_IND + ri) |= USB_Bootloader_ARB_EPX_CFG_IN_DATA_RDY;
                    #endif  /* USB_Bootloader_MANUAL_IN_EP_ARM */
                                                                        ...
                                                                        
{
    ...
    if((epNumber > USB_Bootloader_EP0) && (epNumber < USB_Bootloader_MAX_EP) && (pData != NULL))
    {
        ...
        p = (reg8 *)(USB_Bootloader_ARB_RW1_DR_IND + ri);
            ...
            (void) CyDmaTdSetAddress(USB_Bootloader_DmaTd[epNumber],  LO16((uint32)p), LO16((uint32)pData));
            /* Enable the DMA */
            (void) CyDmaChSetInitialTd(USB_Bootloader_DmaChan[epNumber], USB_Bootloader_DmaTd[epNumber]);
            (void) CyDmaChEnable(USB_Bootloader_DmaChan[epNumber], 1u);
            ...
            * (reg8 *)(USB_Bootloader_ARB_EP1_CFG_IND + ri) &= ((uint8)(~USB_Bootloader_ARB_EPX_CFG_DMA_REQ));
            ...
            /* Enable DMA in mode3 for transferring data */
            (void) CyDmaChDisable(USB_Bootloader_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USB_Bootloader_DmaTd[epNumber], length, USB_Bootloader_DmaTd[epNumber],
                                                                                TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USB_Bootloader_DmaTd[epNumber],  LO16((uint32)p), LO16((uint32)pData));
...
uint8 USB_Bootloader_GetEPAckState(uint8 epNumber) 
{
    ...
    {
        ri = ((epNumber - USB_Bootloader_EP1) << USB_Bootloader_EPX_CNTX_ADDR_SHIFT);
        cr = CY_GET_REG8((reg8 *)(USB_Bootloader_SIE_EP1_CR0_IND + ri)) & USB_Bootloader_MODE_ACKD;
    }
...
void USB_Bootloader_SetPowerStatus(uint8 powerStatus) 
{
    ...
    *******************************************************************************/
    ...
    {
        return((0u != (CY_GET_REG8(USB_Bootloader_VBUS_PS_PTR) & USB_Bootloader_VBUS_MASK)) ? 1u : 0u);
    }
