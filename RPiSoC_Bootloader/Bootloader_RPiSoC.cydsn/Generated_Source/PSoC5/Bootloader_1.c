...
const uint32 CYCODE *Bootloader_1_SizeBytesAccess = (const uint32 CYCODE *)(&Bootloader_1_SizeBytes);
    ...
    uint8 Bootloader_1_activeApp = Bootloader_1_MD_BTLDB_ACTIVE_NONE;
#else
...
static uint8    Bootloader_1_Calc8BitFlashSum(uint32 start, uint32 size) CYSMALL \
                                    ;
#if(!CY_PSOC4)
static uint8    Bootloader_1_Calc8BitEepromSum(uint32 start, uint32 size) CYSMALL \
                                    ;
#endif /* (!CY_PSOC4) */

static void     Bootloader_1_HostLink(uint8 timeOut) \
                                    ;

static void     Bootloader_1_LaunchApplication(void) CYSMALL \
                                    ;

static cystatus Bootloader_1_ValidateBootloadable(uint8 appId) CYSMALL \
                                    ;

static uint32   Bootloader_1_GetMetadata(uint8 fieldName, uint8 appId)\
                                    ;
...
*
*******************************************************************************/
static uint8 Bootloader_1_Calc8BitFlashSum(uint32 start, uint32 size) \
                CYSMALL 
{
    uint8 CYDATA sum = 0u;
    ...
    {
        size--;
        sum += Bootloader_1_GET_CODE_BYTE(start + size);
    }
    ...
    /*******************************************************************************
    * Function Name: Bootloader_1_Calc8BitEepromSum
    ********************************************************************************
    *
    * Summary:
    *  This computes the 8 bit sum for the provided number of bytes contained in
    ...
    *
    *******************************************************************************/
    static uint8 Bootloader_1_Calc8BitEepromSum(uint32 start, uint32 size) \
                    CYSMALL 
    {
        uint8 CYDATA sum = 0u;
        ...
        {
            size--;
            sum += Bootloader_1_GET_EEPROM_BYTE(start + size);
        }
...
void Bootloader_1_Start(void) CYSMALL 
{
    ...
    /* Identify active bootloadable application */
        ...
        if(Bootloader_1_MD_BTLDB_ACTIVE_VALUE(0u) == Bootloader_1_MD_BTLDB_IS_ACTIVE)
        {
            Bootloader_1_activeApp = Bootloader_1_MD_BTLDB_ACTIVE_0;
        }
        else if (Bootloader_1_MD_BTLDB_ACTIVE_VALUE(1u) == Bootloader_1_MD_BTLDB_IS_ACTIVE)
        {
    ...
    ***********************************************************************/
        ...
        calcedChecksum = Bootloader_1_Calc8BitFlashSum(Bootloader_1_MD_BTLDR_ADDR_PTR,
    ...
    ***********************************************************************/
    tmpStatus = Bootloader_1_ValidateBootloadable(Bootloader_1_activeApp);

    if ((Bootloader_1_GET_RUN_TYPE == Bootloader_1_START_BTLDR) ||
...
void CyBtldr_CheckLaunch(void) CYSMALL 
{
    ...
    *******************************************************************************/
    ...
    if (Bootloader_1_GET_RUN_TYPE == Bootloader_1_START_APP)
    {
        ...
        *******************************************************************************/
        if(0u != Bootloader_1_GetMetadata(Bootloader_1_GET_METADATA_BTLDB_ADDR, Bootloader_1_activeApp))
        {
            /* Never return from this method */
            Bootloader_1_LaunchBootloadable(Bootloader_1_GetMetadata(Bootloader_1_GET_METADATA_BTLDB_ADDR,
...
static cystatus Bootloader_1_ValidateBootloadable(uint8 appId) CYSMALL \

    {
        ...
        uint32 CYDATA end   = Bootloader_1_FIRST_APP_BYTE(appId) +
                                Bootloader_1_GetMetadata(Bootloader_1_GET_METADATA_BTLDB_LENGTH,
                                                       appId);
        ...
        uint8  CYDATA calcedChecksum = 0u;
            ...
            if(Bootloader_1_MD_BTLDB_VERIFIED_VALUE(appId) == Bootloader_1_MD_BTLDB_IS_VERIFIED)
            {
        ...
        ***************************************************************************/
        ...
        if((calcedChecksum != Bootloader_1_MD_BTLDB_CHECKSUM_VALUE(appId)) ||
           (0u == valid))
        {
            return(CYRET_BAD_DATA);
        }
                    ...
                    if((Bootloader_1_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 1u))
                    {
                        ...
                        else if(CYRET_SUCCESS == Bootloader_1_ValidateBootloadable(btldrData))
                        {
                                ...
                                (void) memcpy(&packetBuffer[Bootloader_1_DATA_ADDR],
                                            ((uint8  CYCODE *) (Bootloader_1_META_BASE(btldrData))), 56);
                            #else
                                (void) memcpy(&packetBuffer[Bootloader_1_DATA_ADDR],
                                            (uint8 *) Bootloader_1_META_BASE(btldrData), 56u);
                            #endif  /* (CY_PSOC3) */
            ...
            ***************************************************************************/
                ...
                case Bootloader_1_COMMAND_REPORT_SIZE:
                    ...
                    if((Bootloader_1_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 1u))
                    {
                        ...
                        if(btldrData < Bootloader_1_NUM_OF_FLASH_ARRAYS)
                        {
                            #if (1u == Bootloader_1_NUM_OF_FLASH_ARRAYS)
                                uint16 CYDATA startRow = (uint16)*Bootloader_1_SizeBytesAccess / CYDEV_FLS_ROW_SIZE;
                            #else
                                uint16 CYDATA startRow = 0u;
                            #endif  /* (1u == Bootloader_1_NUM_OF_FLASH_ARRAYS) */

                            packetBuffer[Bootloader_1_DATA_ADDR]      = LO8(startRow);
                            packetBuffer[Bootloader_1_DATA_ADDR + 1u] = HI8(startRow);
                            packetBuffer[Bootloader_1_DATA_ADDR + 2u] = LO8(CY_FLASH_NUMBER_ROWS - 1u);
                            packetBuffer[Bootloader_1_DATA_ADDR + 3u] = HI8(CY_FLASH_NUMBER_ROWS - 1u);
                    ...
                    case Bootloader_1_COMMAND_APP_STATUS:
                        ...
                        if((Bootloader_1_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 1u))
                        {
                            ...
                            packetBuffer[Bootloader_1_DATA_ADDR + 1u] =
                                (uint8)Bootloader_1_MD_BTLDB_ACTIVE_VALUE(btldrData);
                    ...
                    if(dataOffset == pktSize)
                    {
                        ...
                        dataOffset = ((uint16)((uint16)packetBuffer[Bootloader_1_DATA_ADDR + 2u] << 8u)) |
                            ...
                            if(0u == clearedMetaData)
                            {
                                ...
                                #if(CY_PSOC4)
                                    (void) CySysFlashWriteRow(Bootloader_1_MD_ROW, erase);
                                #else
                                    (void) CyWriteRowFull((uint8)  Bootloader_1_MD_FLASH_ARRAY_NUM,
                                                          (uint16) Bootloader_1_MD_ROW,
                                                                    erase,
                            ...
                            if(Bootloader_1_activeApp < Bootloader_1_MD_BTLDB_ACTIVE_NONE)
                            {
                                ...
                                uint16 firstRow = (uint16) 1u +
                                    (uint16) Bootloader_1_GetMetadata(Bootloader_1_GET_METADATA_BTLDR_LAST_ROW,
                                                                          Bootloader_1_activeApp);
                                ...
                                * then: (2u - Bootloader_1_activeApp).
                                *
                                ...
                                *******************************************************************************/
                                uint16 lastRow = (firstRow - 1u) +
                                                  ...
                                                  ((uint16)2u - (uint16)Bootloader_1_activeApp));
                                ...
                                *******************************************************************************/
                                if(((row >= firstRow) && (row <= lastRow)) ||
                                   ((btldrData == Bootloader_1_MD_FLASH_ARRAY_NUM) &&
                                   (dataOffset == Bootloader_1_MD_ROW_NUM(Bootloader_1_activeApp))))
                                {
                ...
                if((Bootloader_1_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 3u))
                {
                    ...
                    uint16 CYDATA rowNum = ((uint16)((uint16)packetBuffer[Bootloader_1_DATA_ADDR + 2u] << 8u)) |
                           ...
                           (btldrData <= Bootloader_1_LAST_EE_ARRAYID))
                        {
                            ...
                            checksum = Bootloader_1_Calc8BitEepromSum(rowAddr, CYDEV_EEPROM_ROW_SIZE);
                        }
                        else
                        {
                            /* FLASH */
                            ...
                            checksum = Bootloader_1_Calc8BitFlashSum(rowAddr, CYDEV_FLS_ROW_SIZE);
                        }
                    ...
                    * correct.
                     ******************************************************************************/
                    if((Bootloader_1_MD_FLASH_ARRAY_NUM == btldrData) &&
                       (Bootloader_1_CONTAIN_METADATA(rowNum)))
                    {
                        checksum -= Bootloader_1_MD_BTLDB_ACTIVE_VALUE  (Bootloader_1_GET_APP_ID(rowNum));
                        checksum -= Bootloader_1_MD_BTLDB_VERIFIED_VALUE(Bootloader_1_GET_APP_ID(rowNum));
                    }
...
*     Bootloader_1_GET_METADATA_BTLDB_ADDR
*     Bootloader_1_GET_METADATA_BTLDR_LAST_ROW
*     Bootloader_1_GET_METADATA_BTLDB_LENGTH
*     Bootloader_1_GET_METADATA_BTLDR_APP_VERSION
*     Bootloader_1_GET_METADATA_BTLDB_APP_VERSION
*     Bootloader_1_GET_METADATA_BTLDB_APP_ID
*     Bootloader_1_GET_METADATA_BTLDB_APP_CUST_ID
*
*  appId:
...
static uint32 Bootloader_1_GetMetadata(uint8 fieldName, uint8 appId)
{
    ...
    switch (fieldName)
    {
    case Bootloader_1_GET_METADATA_BTLDB_APP_CUST_ID:
        fieldPtr  = Bootloader_1_MD_BTLDB_APP_CUST_ID_OFFSET(appId);
        fieldSize = 4u;
        break;

    case Bootloader_1_GET_METADATA_BTLDR_APP_VERSION:
        fieldPtr  = Bootloader_1_MD_BTLDR_APP_VERSION_OFFSET(appId);
        break;

    case Bootloader_1_GET_METADATA_BTLDB_ADDR:
    ...
    case Bootloader_1_GET_METADATA_BTLDR_LAST_ROW:
        fieldPtr  = Bootloader_1_MD_BTLDR_LAST_ROW_OFFSET(appId);
        break;

    case Bootloader_1_GET_METADATA_BTLDB_LENGTH:
    ...
    case Bootloader_1_GET_METADATA_BTLDB_APP_VERSION:
        fieldPtr  = Bootloader_1_MD_BTLDB_APP_VERSION_OFFSET(appId);
        break;

    case Bootloader_1_GET_METADATA_BTLDB_APP_ID:
