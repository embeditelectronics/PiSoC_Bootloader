/*******************************************************************************
* File Name: USB_Bootloader_hid.c
* Version 2.80
*
* Description:
...
volatile uint8 USB_Bootloader_hidProtocol[USB_Bootloader_MAX_INTERFACES_NUMBER];  /* HID device protocol status */
volatile uint8 USB_Bootloader_hidIdleRate[USB_Bootloader_MAX_INTERFACES_NUMBER];  /* HID device idle reload value */
volatile uint8 USB_Bootloader_hidIdleTimer[USB_Bootloader_MAX_INTERFACES_NUMBER]; /* HID device idle rate value */
...
/*******************************************************************************
* Function Name: USB_Bootloader_UpdateHIDTimer
********************************************************************************
*
* Summary:
...
{
    uint8 stat = USB_Bootloader_IDLE_TIMER_INDEFINITE;

    if(USB_Bootloader_hidIdleRate[interface] != 0u)
    {
        if(USB_Bootloader_hidIdleTimer[interface] > 0u)
        {
            USB_Bootloader_hidIdleTimer[interface]--;
            stat = USB_Bootloader_IDLE_TIMER_RUNNING;
        }
        else
        {
            USB_Bootloader_hidIdleTimer[interface] = USB_Bootloader_hidIdleRate[interface];
            stat = USB_Bootloader_IDLE_TIMER_EXPIRED;
        }
    }
...
/*******************************************************************************
* Function Name: USB_Bootloader_GetProtocol
********************************************************************************
*
* Summary:
...
{
    return(USB_Bootloader_hidProtocol[interface]);
}


/*******************************************************************************
* Function Name: USB_Bootloader_DispatchHIDClassRqst
********************************************************************************
*
* Summary:
...
uint8 USB_Bootloader_DispatchHIDClassRqst(void) 
{
    ...
    {   /* Control Read */
        switch (CY_GET_REG8(USB_Bootloader_bRequest))
        {
            case USB_Bootloader_GET_DESCRIPTOR:
                if (CY_GET_REG8(USB_Bootloader_wValueHi) == USB_Bootloader_DESCR_HID_CLASS)
                {
                    USB_Bootloader_FindHidClassDecriptor();
                    if (USB_Bootloader_currentTD.count != 0u)
                    {
                        requestHandled = USB_Bootloader_InitControlRead();
                    }
                }
                else if (CY_GET_REG8(USB_Bootloader_wValueHi) == USB_Bootloader_DESCR_HID_REPORT)
                {
                    USB_Bootloader_FindReportDescriptor();
                    if (USB_Bootloader_currentTD.count != 0u)
                    {
                        requestHandled = USB_Bootloader_InitControlRead();
                    }
                ...
                break;
            case USB_Bootloader_HID_GET_REPORT:
                USB_Bootloader_FindReport();
                if (USB_Bootloader_currentTD.count != 0u)
                {
                    requestHandled = USB_Bootloader_InitControlRead();
                }
            ...
            case USB_Bootloader_HID_GET_IDLE:
                /* This function does not support multiple reports per interface*/
                /* Validate interfaceNumber and Report ID (should be 0) */
                if( (interfaceNumber < USB_Bootloader_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(USB_Bootloader_wValueLo) == 0u ) ) /* Do not support Idle per Report ID */
                {
                    USB_Bootloader_currentTD.count = 1u;
                    USB_Bootloader_currentTD.pData = &USB_Bootloader_hidIdleRate[interfaceNumber];
                    requestHandled  = USB_Bootloader_InitControlRead();
                }
                break;
            case USB_Bootloader_HID_GET_PROTOCOL:
                /* Validate interfaceNumber */
                if( interfaceNumber < USB_Bootloader_MAX_INTERFACES_NUMBER)
                {
                    USB_Bootloader_currentTD.count = 1u;
                    USB_Bootloader_currentTD.pData = &USB_Bootloader_hidProtocol[interfaceNumber];
                    requestHandled  = USB_Bootloader_InitControlRead();
                }
                break;
    ...
    {   /* Control Write */
        switch (CY_GET_REG8(USB_Bootloader_bRequest))
        {
            case USB_Bootloader_HID_SET_REPORT:
                USB_Bootloader_FindReport();
                if (USB_Bootloader_currentTD.count != 0u)
                {
                ...
                break;
            case USB_Bootloader_HID_SET_IDLE:
                /* This function does not support multiple reports per interface */
                /* Validate interfaceNumber and Report ID (should be 0) */
                if( (interfaceNumber < USB_Bootloader_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(USB_Bootloader_wValueLo) == 0u ) ) /* Do not support Idle per Report ID */
                {
                    USB_Bootloader_hidIdleRate[interfaceNumber] = CY_GET_REG8(USB_Bootloader_wValueHi);
                    ...
                    */
                    if(USB_Bootloader_hidIdleRate[interfaceNumber] <
                       USB_Bootloader_hidIdleTimer[interfaceNumber])
                    {
                        /* Set the timer to zero and let the UpdateHIDTimer() API return IDLE_TIMER_EXPIRED status*/
                        USB_Bootloader_hidIdleTimer[interfaceNumber] = 0u;
                    }
                    ...
                    */
                    else if(USB_Bootloader_hidIdleTimer[interfaceNumber] <= 1u)
                    {
                        /* Do nothing.
                    ...
                    {   /* Reload the timer*/
                        USB_Bootloader_hidIdleTimer[interfaceNumber] =
                        USB_Bootloader_hidIdleRate[interfaceNumber];
                    }
                    requestHandled = USB_Bootloader_InitNoDataControlTransfer();
                }
                break;

            case USB_Bootloader_HID_SET_PROTOCOL:
                /* Validate interfaceNumber and protocol (must be 0 or 1) */
                if( (interfaceNumber < USB_Bootloader_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(USB_Bootloader_wValueLo) <= 1u) )
                {
                    USB_Bootloader_hidProtocol[interfaceNumber] = CY_GET_REG8(USB_Bootloader_wValueLo);
                    requestHandled = USB_Bootloader_InitNoDataControlTransfer();
                }
                break;
            default:    /* requestHandled is initialized as FALSE by default */
...
void USB_Bootloader_FindHidClassDecriptor(void) 
{
    ...
    interfaceN = CY_GET_REG8(USB_Bootloader_wIndexLo);
    /* Third entry in the LUT starts the Interface Table pointers */
    /* Now use the request interface number*/
    pTmp = &pTmp[interfaceN + 2u];
    /* USB_DEVICEx_CONFIGURATIONy_INTERFACEz_TABLE */
    pTmp = (const T_USB_Bootloader_LUT CYCODE *) pTmp->p_list;
    /* Now use Alternate setting number */
    pTmp = &pTmp[USB_Bootloader_interfaceSetting[interfaceN]];
    ...
    USB_Bootloader_currentTD.count = *pDescr;
    USB_Bootloader_currentTD.pData = pDescr;
}
...
void USB_Bootloader_FindReportDescriptor(void) 
{
    ...
    USB_Bootloader_currentTD.count =  (((uint16)pDescr[1u] << 8u) | pDescr[0u]);
    USB_Bootloader_currentTD.pData = &pDescr[2u];
}


/*******************************************************************************
* Function Name: USB_Bootloader_FindReport
********************************************************************************
*
* Summary:
...
void USB_Bootloader_FindReport(void) 
{
    ...
    #ifdef USB_Bootloader_FIND_REPORT_CALLBACK
        USB_Bootloader_FindReport_Callback();
    #endif /* USB_Bootloader_FIND_REPORT_CALLBACK */
    
    USB_Bootloader_currentTD.count = 0u;   /* Init not supported condition */
    pTmp = USB_Bootloader_GetConfigTablePtr(USB_Bootloader_configuration - 1u);
    reportType = CY_GET_REG8(USB_Bootloader_wValueHi);
    interfaceN = CY_GET_REG8(USB_Bootloader_wIndexLo);
    ...
    /* USB_DEVICEx_CONFIGURATIONy_INTERFACEz_TABLE*/
    pTmp = (const T_USB_Bootloader_LUT CYCODE *) pTmp->p_list;
    if(interfaceN < USB_Bootloader_MAX_INTERFACES_NUMBER)
    {
        /* Now use Alternate setting number */
        pTmp = &pTmp[USB_Bootloader_interfaceSetting[interfaceN]];
        /* USB_DEVICEx_CONFIGURATIONy_INTERFACEz_ALTERNATEi_HID_TABLE */
        ...
        if((reportType >= USB_Bootloader_HID_GET_REPORT_INPUT) &&
           (reportType <= USB_Bootloader_HID_GET_REPORT_FEATURE))
        {
            /* Get the entry proper TD (IN, OUT or Feature Report Table)*/
            pTmp = &pTmp[reportType - 1u];
            reportType = CY_GET_REG8(USB_Bootloader_wValueLo);    /* Get reportID */
            /* Validate table support by the HID descriptor, compare table count with reportID */
            if(pTmp->c >= reportType)
            {
                pTD = (T_USB_Bootloader_TD *) pTmp->p_list;
                pTD = &pTD[reportType];                          /* select entry depend on report ID*/
                USB_Bootloader_currentTD.pData = pTD->pData;   /* Buffer pointer */
                USB_Bootloader_currentTD.count = pTD->count;   /* Buffer Size */
                USB_Bootloader_currentTD.pStatusBlock = pTD->pStatusBlock;
            }
        }
    }
}
